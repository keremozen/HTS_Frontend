export * from './additional-service.service';
export * from './branch.service';
export * from './city.service';
export * from './contracted-institution-kind.service';
export * from './contracted-institution-staff.service';
export * from './contracted-institution-type.service';
export * from './contracted-institution.service';
export * from './currency.service';
export * from './document-type.service';
export * from './exchange-rate-information.service';
export * from './external.service';
export * from './gender.service';
export * from './hospital-agent-note.service';
export * from './hospital-consultation-document.service';
export * from './hospital-consultation.service';
export * from './hospital-pricer.service';
export * from './hospital-response-type.service';
export * from './hospital-response.service';
export * from './hospital-staff.service';
export * from './hospital-uhbstaff.service';
export * from './hospital.service';
export * from './hospitalization-type.service';
export * from './htstask.service';
export * from './invitation-letter-document.service';
export * from './language.service';
export * from './nationality.service';
export * from './operation.service';
export * from './patient-admission-method.service';
export * from './patient-document.service';
export * from './patient-note.service';
export * from './patient-treatment-process.service';
export * from './patient.service';
export * from './payment-document.service';
export * from './payment-kind.service';
export * from './payment-reason.service';
export * from './payment.service';
export * from './process-cost.service';
export * from './process-kind.service';
export * from './process-relation.service';
export * from './process-type.service';
export * from './process.service';
export * from './proforma.service';
export * from './reject-reason.service';
export * from './sales-method-and-companion-info.service';
export * from './treatment-process-status.service';
export * from './treatment-type.service';
export * from './user.service';
export * from './uss.service';
